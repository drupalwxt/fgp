...
namespace Drupal\fgp\Composer;

use Drupal\fgp\IniEncoder;
use Composer\Script\Event;
use Symfony\Component\Finder\Finder;
use Symfony\Component\Yaml\Yaml;

/**
 * Updates the version number in FGP's component info files.
 */
class ReleaseVersion {
  ...
  public static function execute(Event $event) {
    ...
    // Find all .info files in FGP...except in the installed code base.
    $finder = (new Finder())
      ->name('*.info.yml')
      ->in('.')
      ->exclude('html');
  ...
  protected static function updateMakeFile($version) {
    ...
    foreach ($finder as $make_file) {
      ...
      $make['projects']['fgp']['download']['tag'] = $version;
